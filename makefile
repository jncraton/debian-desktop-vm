...
debian-12-text.img:
	LIBGUESTFS_DEBUG=1 LIBGUESTFS_TRACE=1 virt-builder -v -x debian-12 --output $@ --format raw --root-password password:password --install 'build-essential,git,micro,sqlite3' --hostname aucs --firstboot-command 'useradd -m -p "" user'
